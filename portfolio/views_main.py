...
class PhDetailView(GeneralContextMixin, PhDetailMixin, DetailView):
...
class UserCreateView(GeneralContextMixin, UserDetailMixin, CreateView):
...
class PhCreateView(GeneralContextMixin, PhDetailMixin, CreateView):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return redirect(
                reverse_lazy("portfolio:ph_add_first", args=[ph.pk])
            )


class PhEditView(GeneralContextMixin, PhDetailMixin, UpdateView):
...
class PhCreateConfirmView(GeneralContextMixin, PhDetailMixin, DetailView):
    ...
    def dispatch(self, request, pk, *args, **kwargs):
        if self.request.session.get("creating"):
            ...
            return redirect(reverse_lazy("portfolio:ph_detail_pics", args=[pk]))


class PhDeleteView(GeneralContextMixin, PhDetailMixin, DeleteView):
...
class PhAddPicsView(GeneralContextMixin, PhDetailMixin, DetailView):
    template_name = "portfolio/ph_add_pics.html"
    segment = "edit"
    creating = False


class PhAddFirstPicsView(GeneralContextMixin, PhDetailMixin, DetailView):
